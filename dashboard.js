document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateChart1() {
        ...
        switch (filter) {
            ...
            case 'age_bin':
                ...
                renderBarChart(Array.from(aggregatedData), `${latePaymentsText.toUpperCase()} LATE ACC AGE DISTRIBUTION`, 'COUNT', chart1Container, false, false, 0.1, 'age_bin');
                break;
            case 'education':
                aggregatedData = d3.rollup(filteredData, v => v.length, d => d.education);
                renderBarChart(Array.from(aggregatedData), `${latePaymentsText.toUpperCase()} LATE ACC EDUCATION DISTRIBUTION`, 'COUNT', chart1Container, false, false, 0.1, 'education');
                break;
            default:
    ...
    function updateChart2() {
        ...
        switch (filter) {
            ...
            case 'age_bin':
                aggregatedData = d3.rollup(filteredData, v => v.length, d => d.age_bin);
                renderBarChart(Array.from(aggregatedData), `CREDIT UTILIZATION RATIO - ${filter.toUpperCase()} DISTRIBUTION`, 'COUNT', chart2Container, false, false, 0.1, 'age_bin');
                break;
            case 'education':
                aggregatedData = d3.rollup(filteredData, v => v.length, d => d.education);
                renderBarChart(Array.from(aggregatedData), `CREDIT UTILIZATION RATIO - ${filter.toUpperCase()} DISTRIBUTION`, 'COUNT', chart2Container, false, false, 0.1, 'education');
                break;
            default:
                console.warn('Unknown filter type:', filter);
        }
    ...
    function aggregateDataForMultiLineChart(data, groupBy) {
        ...
        if (groupBy === 'sex') {
            ...
            groups = ['graduate school', 'high school', 'others', 'university', 'unknown'];
        } else {
    ...
    function renderBarChart(data, xLabel, yLabel, container, useGradient = false, isChart2Default = false, barPadding = 0.1, category = null) {
        ...
        const ageGroupColor = d3.scaleOrdinal()
            ...
            .range(['red', 'blue', 'orange', 'black', 'pink', 'purple']);
        ...
        svg.append('g')
            ...
            .append('rect')
            .attr('class', 'bar')
            .attr('x', d => x(d[0]) + (x.bandwidth() - x.bandwidth()) / 2)
            .attr('y', d => y(d[1]))
    ...
    function renderPieChart(data, title, showLegend, container, filter) {
        const containerWidth = container.clientWidth;
        const containerHeight = container.clientHeight;
        ...
        const color = filter === 'marriage' ? maritalStatusColor 
                     ...
                     : filter === 'education' ? educationColor 
